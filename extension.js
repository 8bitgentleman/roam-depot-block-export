...
function searchForRef(uid){
    ...
    let blockInfo = window.roamAlphaAPI.q(queryText, uid)[0][0];
    return blockInfo
}
function renderText(text){
  // searching for block aliases [Use Humor to soften](((RXUbeHekc)))
  let regexp = /\[([^\[\]]+?)\]\(\(\((.+?)\)\)\)/gm
  let blockAliases = [...text.matchAll(regexp)];
  if (blockAliases.length != 0) {
    blockAliases.forEach((a) => {
      // console.log(a[1])
      let text = text.replace(a[0], a[1])
      // console.log(text)
  ...
  if (matches.length != 0) {
    matches.forEach((m) => {
      // console.log(m[1])
      let rev = searchForRef(m[1]).string
      let text = text.replace(m[0], rev)
      // console.log(text)
  ...
  regexp = /\[\[\>\]\]/gm
  let blockQuotes = [...text.matchAll(regexp)];
  if (blockQuotes.length != 0) {
    blockQuotes.forEach((a) => {
      // console.log(a[1])
      let text = text.replace(a[0], '>')
      // console.log(text)
...
function exportBlock(parentUID){
    ...
    let blockInfo = window.roamAlphaAPI.q(queryText, parentUID)[0][0];

    let markdown = expandChildren(blockInfo)
...
export default {
  onload: () => {
    console.log("load block export plugin")
  ...
  onunload: () => {
    console.log("unload block export plugin")
